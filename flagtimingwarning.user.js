...
with_jquery(function($) {
    $('document').ready(function() {
        ...
        $.get("https://api.stackexchange.com/2.2/posts/" + postsToCheck + "/revisions", {key: "FgWOIKMLFEJQ86U3hbkgDA((", site: window.location.hostname, pagesize: 100, filter: "!*Kb37YAWvnLZdFd."}).done(function(data) {
            ...
            // One warning per post is more than sufficient, remove duplicates
            $('.mod-message').each(function() {
                $(this).find('tr').filter((i,e) => e.innerText.indexOf('Post revised since first flag cast') >= 0).slice(1).remove();
            });
        });
    });
});
